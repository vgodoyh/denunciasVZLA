<x-layouts::admin :title="'Crear Emisor'">
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Crear Emisor</h5>
        </div>

        <div class="card-body">
            <form action="{{ route('emisor.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tipoemisor_id" id="tipoemisor_id" class="form-select @error('tipoemisor_id') is-invalid @enderror">
                            ...
                            @foreach ($tiposEmisor as $tipo)
                                <option value="{{ $tipo->id }}" {{ old('tipoemisor_id') == $tipo->id ? 'selected' : '' }}>
                                    {{ $tipo->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Al menos una fila visible al entrar
        agregarFila();
    });
    </script>
</x-layouts::admin>