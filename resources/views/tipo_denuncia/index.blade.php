<x-layouts::admin :title="'Tipos de Denuncia'">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Tipos de Denuncia</h5>
            <div>
                <a href="{{ route('tipo_denuncia.papelera') }}" class="btn btn-outline-dark me-2">
                    <i class="fas fa-trash"></i> Papelera
                </a>
                <a href="{{ route('tipo_denuncia.create') }}" class="btn btn-dark">
                    <i class="fa-solid fa-plus-circle"></i> Crear tipo de denuncia
                </a>
            </div>
        </div>
                ...
                <table class="table table-hover align-items-center mb-0">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="text-sm text-bold ps-2">Descripción</th>
                            <th class="text-sm text-bold ps-2">Estado</th>
                            <th class="text-sm text-bold text-end">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($tiposDenuncia as $item)
                            <tr>
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->name }}</td>
                                <td>{{ \Illuminate\Support\Str::limit($item->descripcion, 50) }}</td>
                                <td>
                                    @if ($item->activo)
                                ...
                                <td class="text-end">
                                    <div class="action-group" wire:click.stop>
                                        <a href="{{ route('tipo_denuncia.edit', $item->id) }}"
                                           class="act-btn edit"
                                           data-bs-toggle="tooltip" title="Editar">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>

                                        <form action="{{ route('tipo_denuncia.destroy', $item->id) }}" method="post" class="form-eliminar d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-muted">No hay tipos de denuncia registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
