<x-layouts::admin :title="'Papelera - Tipos de Denuncia'">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            @if ($tiposDenuncia->count() > 0)
                <form action="{{ route('tipo_denuncia.restoreAll') }}" method="POST">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="btn btn-dark">
                        <i class="fas fa-trash-restore me-1"></i> Restaurar todos
                    </button>
                </form>
            @endif
        </div>
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="action-group" wire:click.stop>
                                        <form action="{{ route('tipo_denuncia.restore', $item->id) }}" method="post">
                                            @csrf
                                            @method('PUT')
